...
if file_input == "MS":
    data = pd.read_csv("normalized_data_MS.csv")
elif file_input == "LS":
    data = pd.read_csv("normalized_data_LS.csv")
elif file_input == "tops":
    data = pd.read_csv("normalized_data_tops.csv")
elif file_input == "sites":
    data = pd.read_csv("normalized_data_sites.csv")
...
X = data.iloc[:, :-1]  # AIXO SHA DE ARREGLAR PQ PER TOPS I DATA NO ES AIXI
y = data['Eads/abs']
...
if model_input == "RFR" or model_input == "GBR":
    ...
    if model_input == "RFR":
        ...
        max_features = [None, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0]
...
if file_input in valid_file_inputs and model_input in valid_model_inputs:
    plt.savefig(f'learning_curve_{model_input}_{file_input}.pdf', dpi=300, bbox_inches='tight')
...
ax[0].scatter(y_train, train_predictions, alpha=0.9, c="blue")
ax[0].plot(np.linspace(100,-1000,100), np.linspace(100,-1000,100), color="k")
ax[0].set_ylabel("Predicted $E$ values")
ax[0].set_xlim([-1000, -100])
ax[0].set_ylim([-1000, -100])
ax[0].set_title("Training Set")
ax[0].grid(linestyle='--', linewidth=0.5)
ax[0].text(-950, -170, f'R² = {train_r2:.2f}', fontsize=12, color='k')

ax[1].scatter(y_test, test_predictions, alpha=0.9, c="darkorange")
ax[1].plot(np.linspace(100,-1000,100), np.linspace(100,-1000,100), color="k")
ax[1].set_xlim([-1000, -100])
ax[1].set_ylim([-1000, -100])
ax[1].set_title("Test Set")
ax[1].grid(linestyle='--', linewidth=0.5)
ax[1].text(-950, -170, f'R² = {test_r2:.2f}', fontsize=12, color='k')

fig.text(0.52, 0.04, 'Real $E$ values', ha='center', va='center')
plt.tight_layout(rect=[0, 0.04, 1, 1])

if file_input in valid_file_inputs and model_input in valid_model_inputs:
    plt.savefig(f'regression_{model_input}_{file_input}.pdf', dpi=300, bbox_inches='tight')
