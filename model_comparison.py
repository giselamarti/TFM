...
# set0: ALL DATA (1+2+3)
# set1: ATOMISTIC FEATURES
# set2: STRUCTURAL FEATURES
# set3: ELECTRONIC FEATURES

if file_name == "normalized_data_MS.csv" or file_name == "normalized_data_LS.csv":
    set0 = data.iloc[:, :-1]
    set1 = data.iloc[:, 3:10]
    set2 = pd.concat([data.iloc[:, 0:3], data.iloc[:, 10], data.iloc[:, 12], data.iloc[:, 18]], axis=1)
    set3 = pd.concat([data.iloc[:, 11], data.iloc[:, 13:18]], axis=1)
elif file_name == "normalized_data_sites.csv" or file_name == "normalized_data_tops.csv":
    set0 = pd.concat([data.iloc[:, 0:12], data.iloc[:, 13:20]], axis=1)
    set1 = data.iloc[:, 13:20]
    set2 = pd.concat([data.iloc[:, 0:4], data.iloc[:, 5], data.iloc[:, 11]], axis=1)
    set3 = pd.concat([data.iloc[:, 4], data.iloc[:, 6:11]], axis=1)
else:
...
plt.tight_layout()
plt.savefig('subset_maes_tops.pdf', dpi=300, bbox_inches='tight') # Save plot
...
plt.tight_layout()
plt.savefig('set0_maes_tops.pdf', dpi=300, bbox_inches='tight') # Save plot
